//
//  EditInventoryNewViewController.swift
//  Lekhha
//
...
class EditInventoryNewViewController: UIViewController,UITextFieldDelegate, UIImagePickerControllerDelegate & UINavigationControllerDelegate {
    ...
    @IBOutlet weak var storageLoc1TF: UITextField!
    @IBOutlet weak var storageLoc2TF: UITextField!
...
//                 self.dismiss(animated: true, completion: nil)
    ...
    func get_StockUnitMaster_API_Call() {
        ...
        let urlStr = Constants.BaseUrl + stockUnitNewUrl + accountID
...
//              sender.setTitle(item, for: .normal) //9
    ...
    @IBAction func giveAwatyBtn(_ sender: Any) {
    }
...
//        let pickerSize : CGSize = picker.sizeThatFits(CGSize.zero)
   ...
   @objc func doneBtnTap(_ sender: UIButton) {
    ...
    @IBAction func categoryBtnTapped(_ sender: UIButton) {
        dropDown.dataSource = ["test","test1"]
        dropDown.anchorView = sender as! AnchorView
        dropDown.bottomOffset = CGPoint(x: 0, y: (sender as AnyObject).frame.size.height)
            dropDown.show() //7
            dropDown.selectionAction = { [weak self] (index: Int, item: String) in //8
              guard let _ = self else { return }
                self?.categoryBtn.setTitle(item, for: UIControl.State.normal)
            }
    }
    @IBAction func subCategoryBtnTapped(_ sender: UIButton) {
        ...
        dropDown.bottomOffset = CGPoint(x: 0, y: (sender as AnyObject).frame.size.height)
            ...
            dropDown.selectionAction = { [weak self] (index: Int, item: String) in //8
              guard let _ = self else { return }
                self?.subCategoryBtn.setTitle(item, for: UIControl.State.normal)
            }
    }
    @IBAction func onClickVendormaster(_ sender: Any) {
    }
    @IBAction func onClickStockUnitMaster(_ sender: Any) {
    }
...
//            self.view.makeToast("app.SomethingWentToWrongAlert".localize())
    ...
    func get_storageLocationMaster_API_Call() {
        ...
        let urlStr = Constants.BaseUrl + StorageLocationNewUrl + accountID
...
//        accountID = "5f8970b099c2f076c8c54af6"
                ...
                let URLString_loginIndividual = Constants.BaseUrl + vendorMasterNewUrl + accountID as String
        serviceVC.requestGETURL(strURL:URLString_loginIndividual, success: {(result) in
    ...
    func getEditDetailsAPI()  {
            ...
            let URLString_loginIndividual = Constants.BaseUrl + editCurrentInventoryNew + prodIDStr
        serviceVC.requestGETURL(strURL:URLString_loginIndividual, success: {(result) in
...
//                                        self.getModifiedListAPI()
    ...
    func updateInventoryData()
    {
        ...
        let stockQuan = editInventoryResult[0].productDict?.stockQuantity ?? 0
      ...
      //  selectedCategoryID = (editInventoryResult[0].productDict?.category)?? <#default value#>
