...
protocol sendPropDetails {
    func sendPropertyDetails(data:NSArray,idStr:String)
    
}


class PropertiesShowVC: UIViewController,UITextFieldDelegate,UIScrollViewDelegate {
...
//        let defaults = UserDefaults.standard
        ...
        if let date = dateFormatter.date(from:billDateStr ?? "") {
...
//                self.getSubCategoriesDataFromServer()
    ...
    var storageLocArr=[String]()
        for obj in storageLocationArr {
            let aa=obj as? NSDictionary
            if aa?.value(forKey: "hierachyLevel")as? Int==1{
                
                storageLocArr.append(aa?.value(forKey: "slocName")as? String ?? "")
                storageLocationIdsArr.append(aa?.value(forKey: "_id")as? String ?? "")
                storageLocationParentArr.append(aa?.value(forKey: "parentLocation")as? String ?? "")
            }
    ...
    @IBAction func onClickStorageLocation2Button(_ sender: UIButton) {
        ...
        for obj in storageLocationArr1 {
            let aa=obj as? NSDictionary
                ...
                storageLocArr.append(aa?.value(forKey: "slocName")as? String ?? "")
                storageLocation1IdsArr.append(aa?.value(forKey: "_id")as? String ?? "")
                storageLocation1ParentArr.append(aa?.value(forKey: "parentLocation")as? String ?? "")
        }
    ...
    @IBAction func onClickStorageLocation3Button(_ sender: UIButton) {
        ...
        for obj in storageLocationArr2 {
            let aa=obj as? NSDictionary
                ...
                storageLocArr.append(aa?.value(forKey: "slocName")as? String ?? "")
                storageLocation2IdsArr.append(aa?.value(forKey: "_id")as? String ?? "")
                storageLocation2ParentArr.append(aa?.value(forKey: "parentLocation")as? String ?? "")
        }
                    ...
                    if message == "SUCCESS" {
                        ...
                        if respVo.result!.count > 0 {
                            ...
                            if self.storageLocationArr.count>0
                            {
                                ...
                                self.storageLocation1Button.setTitle(aaaa.value(forKey: "slocName")as? String ?? "", for: UIControl.State.normal)
                            ...
                            if self.hierachyLevel=="2"
                              {
                                ...
                                if self.storageLocationArr1.count>0
                                {
                                    ...
                                    self.storageLocation2Button.setTitle(aaaa.value(forKey: "slocName")as? String ?? "", for: .normal)
                                    self.parentLocation = aaaa.value(forKey: "slocName")as? String ?? ""
                                    self.hierachyLevel = "3"
                            ...
                            else if self.hierachyLevel=="3"
                            {
                                ...
                                if self.storageLocationArr2.count>0
                                {
                                    if self.storageLocationArr2.value(forKey: "hierachyLevel")as? Int == 3
                                    {
                                        ...
                                        self.storageLocation3Button.setTitle(aaaa.value(forKey: "slocName")as? String ?? "", for: .normal)
        ...
        for j in 0...myProductsArray.count {
                ...
                for i in 0..<4 {
                        ...
                        if(i == 0){
                            ...
                            subView.frame = CGRect(x: xValue, y: yValue, width: 50, height: 40)
                            dataScrollView.addSubview(subView)
                            
                            let sepLbl2 = UILabel()
                            sepLbl2.frame = CGRect(x: 0 , y: 0, width: 1, height: subView.frame.size.height)
                            sepLbl2.backgroundColor = .lightText
                            subView.addSubview(sepLbl2)
                            
                            let checkBoxBtn = UIButton()
                            checkBoxBtn.frame = CGRect(x: 12.5, y: 7.5, width: 25, height: 25)
                            checkBoxBtn.setImage(UIImage.init(named: "checkBoxInactive"), for: .normal)
                            subView.addSubview(checkBoxBtn)
                            
                            if(j == 0){
                                checkBoxBtn.addTarget(self, action: #selector(checkBoxAllBtnTap), for: .touchUpInside)
                                ...
                                subView.backgroundColor = hexStringToUIColor(hex: "0078fa")

                            }else{
                                checkBoxBtn.addTarget(self, action: #selector(checkBoxBtnTap), for: .touchUpInside)
                                checkBoxBtn.tag = j - 1
                                
                                let dataDict = myProductsArray.object(at: j-1) as! NSDictionary
                                let isActiveStr = dataDict.value(forKey: "isActive") as? String
                            ...
                            subView.frame = CGRect(x: xValue, y: yValue, width: 150, height: 40)
                            dataScrollView.addSubview(subView)
                            
                            
                            if(j == 0){
                                
                                let headerTxtLbl = UILabel()
                                headerTxtLbl.frame = CGRect(x: 5, y: 0, width: subView.frame.size.width - 10, height: subView.frame.size.height)
                                headerTxtLbl.font = UIFont.init(name: kAppFontMedium, size: 14)
                                headerTxtLbl.textColor = .white
                                headerTxtLbl.textAlignment = .center
                                subView.addSubview(headerTxtLbl)
                                
//                                headerTxtLbl.text = titleStr
                                headerTxtLbl.textColor = hexStringToUIColor(hex: "232c51")

                                if(i == 1){
                                    headerTxtLbl.text = "Item Name"
                                    subView.backgroundColor = hexStringToUIColor(hex: "0078fa")
                                    
                                }else if(i == 2){
                                    headerTxtLbl.text = selectedBillPriceScan
                                    subView.backgroundColor = hexStringToUIColor(hex: "0078fa")


                                }else{
                                    headerTxtLbl.text = "Quantity"
                                    subView.backgroundColor = hexStringToUIColor(hex: "0078fa")
                                ...
                                let idTag1 : String = "1001" + StringTagValue
                                let idTagValue1: Int? = Int(idTag1)
                                
                                let idTag2 : String = "1002" + StringTagValue
                                let idTagValue2: Int? = Int(idTag2)

                                var headerTxtTF = UITextField()
                                headerTxtTF.frame = CGRect(x: 5, y: 0, width: subView.frame.size.width - 10, height: subView.frame.size.height)
                                headerTxtTF.font = UIFont.init(name: kAppFontMedium, size: 14)
                                headerTxtTF.textColor = .white
                                headerTxtTF.textAlignment = .center
                                headerTxtTF.delegate = self
                                subView.addSubview(headerTxtTF)
                                
                                let dataDict = myProductsArray.object(at: j-1) as! NSDictionary
                                
                                if(i == 1){
                                    var titleStr = ""
                                    titleStr = dataDict.value(forKey: "productName") as! String
                                    headerTxtTF.tag = idTagValue ?? 0
                                    headerTxtTF.text = titleStr
                                }else if(i == 2){
                                    if selectedBillPriceScan=="Price Per Unit"
                                    {
                                        if let pb = dataDict.value(forKey: "unitPrice") as? String {
                                            var titleStr = ""
                                            
                                            titleStr = dataDict.value(forKey: "unitPrice") as! String
                                                ...
                                                alertButton.frame = CGRect(x: subView.frame.size.width - 40, y: 0, width: 30, height: 30)
                                                alertButton.setImage(UIImage(named: "warn.png"), for: .normal)
                                                alertButton.addTarget(self, action: #selector(onClickWarnButton), for: .touchUpInside)
                                                subView.addSubview(alertButton)
                                                }
                                            ...
                                            titleStr = dataDict.value(forKey: "unitPrice") as? Float ?? Float()
                                                if dataDict.value(forKey: "unitPricewarn")as? Bool==false
                                                {
                                            ...
                                            headerTxtTF.text = String(titleStr)
                                    ...
                                    {
                                        if let pbbb = dataDict.value(forKey: "price") as? String {
                                            var titleStr = ""
                                            titleStr = dataDict.value(forKey: "price") as! String
                                            if dataDict.value(forKey: "pricewarn")as? Bool==false
                                            {
                                            ...
                                            var titleStr = Float()
                                            titleStr = dataDict.value(forKey: "price") as? Float ?? Float()
                                            if dataDict.value(forKey: "pricewarn")as? Bool==false
                                            {
                                    ...
                                    headerTxtTF.keyboardType = UIKeyboardType.decimalPad
                                    headerTxtTF.tag = idTagValue1 ?? 0
                                }else{
                                    
                                    if let pbss = dataDict.value(forKey: "stockQuantity") as? String {
                                        var titleStr = ""
                                        titleStr = dataDict.value(forKey: "stockQuantity") as! String
                                        headerTxtTF.keyboardType = UIKeyboardType.decimalPad
                                        headerTxtTF.tag = idTagValue2 ?? 0
                                        if dataDict.value(forKey: "stockwarn")as? Bool==false
                                        {
                                        ...
                                        var titleStr = Float()
                                        titleStr = dataDict.value(forKey: "stockQuantity") as? Float ?? Float()
                                        headerTxtTF.keyboardType = UIKeyboardType.decimalPad
                                        headerTxtTF.tag = idTagValue2 ?? 0
                                        if dataDict.value(forKey: "stockwarn")as? Bool==false
                                        {
                                ...
                                headerTxtTF.textColor = hexStringToUIColor(hex: "232c51")
                            }
                        }
                        
                        let sepLbl = UILabel()
                        sepLbl.frame = CGRect(x: 0 , y: subView.frame.size.height - 1, width: subView.frame.size.width, height: 1)
                        sepLbl.backgroundColor = .lightText
                    sepLbl.backgroundColor = hexStringToUIColor(hex: "232c51")
                        subView.addSubview(sepLbl)
                        
                        let sepLbl2 = UILabel()
                        sepLbl2.frame = CGRect(x: subView.frame.size.width - 1 , y: 0, width: 1, height: subView.frame.size.height)
                        sepLbl2.backgroundColor = .lightText
                    sepLbl2.backgroundColor = hexStringToUIColor(hex: "232c51")
                        subView.addSubview(sepLbl2)
                    
                    if(j == 0){
                        
                        let sepLbl = UILabel()
                        sepLbl.frame = CGRect(x: 0 , y: 0, width: subView.frame.size.width, height: 1)
                        sepLbl.backgroundColor = .lightText
                        subView.addSubview(sepLbl)
                        ...
                        xValue = xValue + Int(subView.frame.size.width)
...
//                    }
      ...
      let doneBtn1 = UIButton()
        doneBtn1.frame = CGRect(x: 0, y: self.dataScrollView.frame.origin.y+self.dataScrollView.frame.size.height+20, width: self.view.frame.size.width, height: 40)
        doneBtn1.setTitle("Done", for: .normal)
       // backVieww.addSubview(doneBtn1)
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
       ...
       productArrayPosition = textField.tag%100
        ...
        if(productTagValue == 1001){ //Product Name
            ...
            if aa>2 {
                if oldText.count>newString.count
                {
                    return true
                }
                else
